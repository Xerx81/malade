...
PATIENT_FIELDS = [
    ...
    'somme_payee_10', 'date_paiement_10', 'weight', 'commentaire_taille', 'commentaire_poids', 
]
...
def init_db():
    ...
    columns_definition = ',\n'.join([
        f"{field} REAL" if field in numeric_fields else f"{field} TEXT"
    ...
    if request.method == 'POST':
        ...
        if patient_id:
            ...
            try:
                ...
                flash("Erreur lors de la mise à jour du patient. Veuillez essayer encore.", 'error')
            ...
            try:
                ...
                traceback.print_exc()
                conn.rollback()  # Rollback changes if an error occurs
                flash("Eerreur lors de l'ajout du patient. Veuillez essayer encore.", 'error')
...
def register():
    if request.method == 'POST':
        ...
        try:
            ...
            conn.commit()
            flash('Enregistrement effectué avec succès! Vous pouvez maintenant vous connecter.', 'success')
            return redirect(url_for('login'))
        except sqlite3.IntegrityError:
            flash('Le nom d\'utilisateur ou l\'email est déjà utilisé. Veuillez choisir un autre nom d\'utilisateur ou email.', 'error')
        finally:
            conn.close()
...
def dashboard():
    ...
    c.execute('SELECT id, nom, dateNaissance, age, statut, created_at FROM patients ORDER BY created_at DESC LIMIT 10')
    patients = c.fetchall()
    conn.close()
