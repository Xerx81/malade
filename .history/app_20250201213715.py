...
PATIENT_FIELDS = [
    'nom', 'adresse', 'date_naissance', 'age', 'statut', 'occupation', 'numero_personnel',
    'numero_etablissement', 'nom_conjoint', 'occupation_conjoint', 'numero_tel_conjoint',
    'numero_etablissement_conjoint', 'date_naissance_conjoint', 'age_conjoint', 'ddr', 'dpa',
    'cycle', 'duree', 'g', 'a', 't', 'e', 'p', 'muit', 'type_contraception', 'commentaire_type',
    'duree_contraception', 'commentaire_duree', 'derniere_utilisation', 'commentaire_derniere_utilisation',
    ...
    'sexe_enfant', 'apgar', 'observations_enfant'
]
...
def save_patient():
    try:
        ...
        insert_sql = generate_insert_sql('patients', ['created_at'] + PATIENT_FIELDS)

        # Retrieve values dynamically
        values = [datetime.now()] + get_form_data(data, PATIENT_FIELDS)
...
def liste_patients():
    conn = sqlite3.connect('clinic.db')
    c = conn.cursor()
    c.execute('SELECT id, nom, date_naissance, age, numero_personnel, statut, created_at FROM patients')
    patients = c.fetchall()
    conn.close()
    patients_list = [{
        'id': p[0], 
...
def update_patient_form(patient_id):
    ...
    patient = c.fetchone()
    if patient:
        patient_info = dict(zip(['id'] + PATIENT_FIELDS, patient))
...
def dashboard():
    ...
    return render_template('dashboard.html', patients=patients_list, total_patients=total_patients, patients_today=patients_today, active_patients=active_patients)
